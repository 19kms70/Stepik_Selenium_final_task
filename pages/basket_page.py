...
class BasketPage(BasePage):
    ...
    def see_message_basket_is_empty(self, mask):
        assert self.wait_for_mask_in_element_attribute_text(BasketPageLocators.BASKET_CONTENT,
                                                            mask), f"Do not see message: {mask}"

    def do_not_see_message_item_to_by_now(self, mask):
        assert self.wait_for_mask_not_in_element_attribute_text(BasketPageLocators.BASKET_CONTENT,
                                                                mask), f"see message: {mask}"
